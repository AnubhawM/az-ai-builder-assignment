...
def generate_ppt():
    """
    Deprecated endpoint.
    PPT generation is handled by the workflow API:
    POST /api/workflows/<workflow_id>/generate-ppt
    """
    return jsonify({
        'error': 'Deprecated endpoint. Use workflow endpoints instead.',
        'next_step': 'Use POST /api/workflows/<workflow_id>/generate-ppt with user_id.',
        'status': 'error'
    }), 410
